/**
 * Team 3229
 * CaptureFile.cpp robot interface for 2020 and continued 
 *
...
void CaptureFile::Open(std::string name, bool forWrite) {
  // Setup the name and full path to file << inmportant when we are accessing the RoboRIo file path 
  fileName = name.c_str();
  ...
  // Verify file opened, print debug output on status while cheking if performance on robot is nominal
  if (fileHandle != NULL) {
    if(isWrite) {
